...
lemma number_of_edges_le_aux {n} (G : SimpleGraph (Fin n)) [inst : DecidableRel G.Adj] :
...
lemma remove_vertex_from_complete_graph n :
    remove_vertex (⊤ : SimpleGraph (Fin (n + 1))) = (⊤ : SimpleGraph (Fin n)) := by
  ext x y; simp [remove_vertex]
...
lemma edgeset_card_of_complete_graph n :
  ...
  | succ n iH =>
    ...
    rw [remove_vertex_from_complete_graph] at h
    rw! (castMode := .all) [← h]
...
lemma complete_graph_of_max_number_of_edges {n} (G : SimpleGraph (Fin n)) [DecidableRel G.Adj]
  ...
  | succ n iH =>
    ...
    have h₁ := number_of_edges_le_aux (remove_vertex G)
    ...
    have h₅ : (remove_vertex G).edgeFinset.card = n.choose 2 := by lia
    have h₆ : remove_vertex G = ⊤ := by apply iH _ h₅
